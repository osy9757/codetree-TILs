...
for _ in range(T):
    ...
    for sorted_word in sorted_words:
        if sorted_word[:len(search_word)] == search_word:
            if sorted_words.index(sorted_word)+int(pass_num)-1>=len(sorted_words) or sorted_words[sorted_words.index(sorted_word)+int(pass_num)][:len(search_word)] != search_word:
                print(-1)
                break
            else:
